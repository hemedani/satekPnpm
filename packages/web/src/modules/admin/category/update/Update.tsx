...
export const CategoryUpdate: React.FC<Props> = ({
  isEditable,
  id,
  details,
  history,
  ...
  const { updateCategoryMutate } = useUpdateCategoryMutate(
    { document: "", organizationId: null },
    client
  );
  const methods = useForm<updateCategoryVariables>({
    defaultValues: {
      name: details.name,
  ...
  const onSubmit = methods.handleSubmit(async variables => {
    ...
    if (id && details && path.includes("admin")) {
      ...
      try {
        const result = await updateCategoryMutate({ variables: val });
        history.goBack();
      } catch (err) {
        console.log(err);
      }
